...
async def parse_cnos_list(session, cnos):
    tasks = []
    for cno in cnos:
        task = asyncio.create_task(get_ship_details(session, cno))
        tasks.append(task)
    results = await asyncio.gather(*tasks)
    return results


async def parse_ships_details(cnos):
    async with aiohttp.ClientSession() as session:
        data = await parse_cnos_list(session, cnos)
        ships_details = []
        for result in data:
